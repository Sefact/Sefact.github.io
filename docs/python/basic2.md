...
## while 반복문
for 반복문 이외에도 범용적으로 사용할 수 있는 반복문으로는 while 반복문이 존재합니다.
```python
while 불 표현식:
    문장
```
...
안녕?안녕?안녕?안녕?안녕?안녕?안녕?안녕?안녕?안녕?안녕?안녕?안녕?안녕?안녕?
```
불 표현식에 종료의 조건이 아닌 True가 오는 경우에는 강제 종료 전까지 문장을 계속 반복하게 됩니다.

### 조건을 넣은 while 반복문
```python
i = 0
while i < 5:
    print("{}번째 출력입니다.".format(i + 1))
    i += 1
...
### 상태기반 반복
특정 조건을 만족하면 종료하게 끔 반복문을 설정 가능합니다.
```python
list = [1, 2, 1, 2]
value = 2

# list에 value(=2)가 있다면 반복 아니면 종료
while value in list:
    list.remove(value)
...
### break / continue
반복문 내부에서 사용할 수 있는 키워드로는 `break`, `continue`가 있으며, 각각 반복문을 벗어날 때
그리고 현재 반복을 생략하고 다음으로 넘어갈 떄 사용하는 키워드들 입니다.
```python
i = 0
